...
volatile byte *Adafruit_Keypad::getKeyState(byte key) {
    for(int i=0; i<_numRows*_numCols; i++){
        if(_userKeymap[i] == key){ //우리가 등록한 키패드 배열과 받은 키가 일치할 때 
            return _keystates + i; //keystates에는 NULL값이 들어있음
        }
    }
    return NULL;
}
...
// bool Adafruit_Keypad::isInt(byte key)
// {
//     if(typeid(_KEY_PRESSED == )
//     return(_KEY_PRESSED & typeid(byte).name() == 0; 
// }